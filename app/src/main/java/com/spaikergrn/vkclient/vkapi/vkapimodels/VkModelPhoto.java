...
public class VkModelPhoto implements ILikeAble, VkAttachments.VkModelAttachments {
    ...
    public String getPhotoBySize(final String pSize) {
        String size = pSize;
        if (size.equals(Constants.Parser.EMPTY_STRING)) {
            size = Constants.Parser.PHOTO_604;
        }
        switch (size) {
            case Constants.Parser.PHOTO_1280:
                if (!TextUtils.isEmpty(mPhoto1280)) {
                    return mPhoto1280;
                } else {
                    return getPhotoBySize(Constants.Parser.PHOTO_807);
                }
            case Constants.Parser.PHOTO_807:
                if (!TextUtils.isEmpty(mPhoto807)) {
                    return mPhoto807;
                } else {
                    return getPhotoBySize(Constants.Parser.PHOTO_604);
                }
            case Constants.Parser.PHOTO_604:
                if (!TextUtils.isEmpty(mPhoto604)) {
                    return mPhoto604;
                } else {
                    return getPhotoBySize(Constants.Parser.PHOTO_130);
                }
            case Constants.Parser.PHOTO_130:
                if (!TextUtils.isEmpty(mPhoto130)) {
                    return mPhoto130;
                }
        }
        return null;
    ...
    public String getSmallPhotoForNews() {
        ...
        if (!mPhoto604.equals(Constants.Parser.EMPTY_STRING)) {
            return mPhoto604;
